...
if upload_button:
    ...
    for url in url_list:
        ...
        list_of_urls = get_unique_urls(url, visited_urls)
    ...
    for content in all_urls:
        ...
        unique_urls.extend(urls_in_content)

    unique_urls = list(set(unique_urls))
    st.write("all the Unique URLs ---> ", unique_urls)
    

    
    if (pdf_files != []) & (unique_urls != []):

        for file, url in zip(pdf_files, unique_urls):
            st.sidebar.write(f"Extracting text from PDF file: {file.name}")
            all_pdf_docs = pdf_to_text(pdf_files)
            all_web_docs = web_data_loader(unique_urls)
...
# Add text-to-speech button outside the form
tts_button = st.button("Speak")
if tts_button:
    last_response = st.session_state['user_input'][-1]
